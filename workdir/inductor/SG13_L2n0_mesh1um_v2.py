...
max_cellsize = wavelength_air / (
    sqrt(eps_max) * 20
)  # max cellsize is lambda/20 in medium
...
TopMetal1.AddLinPoly(
    priority=200,
    points=pts,
    norm_dir="z",
    elevation=TopMetal1_zmin,
    length=TopMetal1_thick,
)
...
pts_y = r_[pts_y, 269.000]
pts = np.array([pts_x, pts_y])
TopMetal2.AddLinPoly(
    priority=200,
    points=pts,
    norm_dir="z",
    elevation=TopMetal2_zmin,
    length=TopMetal2_thick,
)
...
pts = np.array([pts_x, pts_y])
TopVia2.AddLinPoly(
    priority=200, points=pts, norm_dir="z", elevation=TopVia2_zmin, length=TopVia2_thick
)
...
port = FDTD.AddLumpedPort(
    1,
    50,
    [-22.2, 0, TopMetal1_zmin],
    [22.2, 10, TopMetal1_zmax],
    "x",
    1.0,
    priority=150,
)
...
Sub.AddBox(
    priority=10,
    start=[box_xmin, box_ymin, Sub_zmin],
    stop=[box_xmax, box_ymax, Sub_zmax],
)
EPI.AddBox(
    priority=10,
    start=[box_xmin, box_ymin, EPI_zmin],
    stop=[box_xmax, box_ymax, EPI_zmax],
)
SiO2.AddBox(
    priority=10,
    start=[box_xmin, box_ymin, SiO2_zmin],
    stop=[box_xmax, box_ymax, SiO2_zmax],
)
...
add_graded_meshlines(
    "z", Sub_zmax, Sub_zmin, -1.5 * refined_cellsize, 1.3, -max_cellsize
)

# EPI: fill equal mesh size
add_equal_meshlines(
    "z", EPI_zmin, EPI_zmax, int(ceil(EPI_thick / refined_cellsize) + 1)
)
...
add_equal_meshlines(
    ...
    TopMetal2_zmax,
    SiO2_zmax,
    int(ceil((SiO2_zmax - TopMetal2_zmax) / refined_cellsize) + 1),
)
...
add_equal_meshlines(
    ...
    TopMetal2_zmin,
    TopMetal2_zmax,
    int(ceil(TopMetal2_thick / refined_cellsize) + 1),
)
add_equal_meshlines(
    "z",
    TopMetal1_zmin,
    TopMetal1_zmax,
    int(ceil(TopMetal1_thick / refined_cellsize) + 1),
)

mesh.AddLine("z", Metal5_zmax)
if Metal5.GetQtyPrimitives() > 0:
    mesh.AddLine("z", Metal5_zmin)

mesh.AddLine("z", Metal4_zmax)
if Metal4.GetQtyPrimitives() > 0:
    mesh.AddLine("z", Metal4_zmin)

mesh.AddLine("z", Metal3_zmax)
if Metal3.GetQtyPrimitives() > 0:
    mesh.AddLine("z", Metal3_zmin)

mesh.AddLine("z", Metal2_zmax)
if Metal2.GetQtyPrimitives() > 0:
    mesh.AddLine("z", Metal2_zmin)

mesh.AddLine("z", Metal1_zmax)
if Metal1.GetQtyPrimitives() > 0:
    mesh.AddLine("z", Metal1_zmin)


# Vias, except for short vias between thin metals
add_equal_meshlines(
    "z", TopVia2_zmin, TopVia2_zmax, int(ceil(TopVia2_thick / refined_cellsize) + 1)
)
if TopVia1.GetQtyPrimitives() > 0:
    add_equal_meshlines(
        "z", TopVia1_zmin, TopVia1_zmax, int(ceil(TopVia1_thick / refined_cellsize) + 1)
    )
if Cont.GetQtyPrimitives() > 0:
    add_equal_meshlines(
        "z", Cont_zmin, Cont_zmax, int(ceil(Cont_thick / refined_cellsize) + 1)
    )
...
add_equal_meshlines(
    "x", geometry_xmin, geometry_xmax, round(geometry_width / refined_cellsize)
)
add_equal_meshlines(
    "y", geometry_ymin, geometry_ymax, round(geometry_height / refined_cellsize)
)
